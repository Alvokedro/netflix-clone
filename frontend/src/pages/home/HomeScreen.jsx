...
import { MOVIE_CATEGORY, ORIGINAL_IMG_BASE_URL, TVSHOW_CATEGORY } from '../../utils/constant';
...
const HomeScreen = () => {
  ...
  if(!trendingContent){
    <div className='h-screen text-white relative'>
      <Navbar />
      <div className='absolute top-0 left-0 w-full h-full bg-black/70 flex items-center justify-center -z-10 shimer' />
    </div>
  }
  return (
    <>
      <div className='relative h-screen text-white'>
        ...
        <div className='absolute top-0 left-0 w-full h-full flex flex-col justify-center px-8 md:px-16 lg:px-32'>
          <div className='bg-gradient-to-b from-black via-transparent to-transparent absolute top-0 left-0 w-full h-full -z-10' aria-hidden='true' />
      ...
      <div className='flex flex-col gap-10 bg-black py-10 '>
          {
            contentType === "movie" ? (
              MOVIE_CATEGORY.map((category) => <MovieTvSlider key={category} category={category}/>)
            ) : (
              TVSHOW_CATEGORY.map((category) => <MovieTvSlider key={category} category={category}/>)
            )
          }
      </div>
    </>
